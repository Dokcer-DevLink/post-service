pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'post-service' // Your DockerHub repository name
...
stage('K8S Manifest Update') {
            steps {
                ...
                dir('gitOpsRepo') {
                    ...
                    sh "sed -i 's/post-service:.*\$/post-service:${currentBuild.number}/' ./eks/post-service.yaml"
                    sh "git add ."
                    sh "git commit -m '[UPDATE] Post-Service K8S ${currentBuild.number} image versioning'"
